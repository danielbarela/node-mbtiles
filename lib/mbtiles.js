...
function MBTiles(uri, callback) {
    ...
    if (Buffer.isBuffer(uri)) {
      this._batchSize = 0;
      sqlitedb.connectWithDbBuffer(uri, function(err, db) {
        if (err) return callback(err);
        mbtiles._db = db;
        mbtiles.filename = db.filePath;
        try {
    ...
    if (typeof uri === 'string') {
        ...
        uri.pathname = qs.unescape ? qs.unescape(uri.pathname) : decodeURIComponent(uri.pathname);
    }
    ...
    mbtiles._db = new sqlitedb(mbtiles.filename, function(err, db) {
      if (err) return callback(err);
      try {
        fs.stat(mbtiles.filename, function(err, stat) {
            if (err) return callback(err);
...
MBTiles.prototype.getInfo = function(callback) {
    ...
    info.filesize = mbtiles._stat ? mbtiles._stat.size : 0;
    ...
    function ensureZooms(info, callback) {
        ...
        var query = mbtiles._db.prepare('SELECT zoom_level FROM tiles WHERE zoom_level = ? LIMIT 1', function(err) {
            if (err) return callback(err.errno === 1  || (err.message && err.message.indexOf('no such table:') === 0) ? null : err, info);
            ...
            async.doUntil(function(cb) {
              query.get(remaining, function(err, row) {
...
MBTiles.prototype.getGeocoderData = function(type, shard, callback) {
    return this._db.get('SELECT data FROM geocoder_data WHERE type = ? AND shard = ?', type, shard, function(err, row) {
        ...
        zlib.inflate(new Buffer(row.data), callback);
    });
};
...
MBTiles.prototype.putGeocoderData = function(type, shard, data, callback) {
    var source = this;
    zlib.deflate(new Buffer(data), function(err, zdata) {
        if (err) return callback(err);
...
MBTiles.prototype.geocoderDataIterator = function(type) {
    ...
    var inflate = function(data, callback) {
        zlibQueue.defer(function(cb) {
            zlib.inflate(new Buffer(data), function(err, buf) {
                callback(err, buf);
                cb();
            });
        });
    ...
    var refillBuffer = function() {
        refilling = true;
        var segmentCount = 0;
        _this._db.each('SELECT shard, data FROM geocoder_data WHERE type = ? ORDER BY shard limit ?,?', type, position, chunkSize, function(err, row, rowCallback) {
            dataQueue.push({row: row, err: err});
            segmentCount += 1;
            sendIfAvailable();
            // async.setImmediate(function() {
              if (rowCallback) { rowCallback(); }
            // });
        }, function() {
            refilling = false;
            if (segmentCount) {
